...
call into ``robotsix_config.history`` — ``apply_update`` for ``PUT /config``,
``rollback``/``read_versions`` for the history surface. They are deliberately
not separable: a UI renders a secret masked, the operator edits a neighbouring
field, the form posts every field back, and a hand-rolled merge writes the mask
over the real credential. That has cost this fleet live credentials before, so
the version history is not re-implemented here — it is ``robotsix_config``'s
``<config>.versions`` JSONL sidecar, shared with robotsix-linkedin and the
future shared config router. The history stores no secret values, and a
rollback restores everything except secrets, carrying live credentials forward.
