"""Claude-auth volume helper — manages ``.credentials.json`` on Docker named volumes.
...
class ClaudeAuthVolumeHelper:
    """Operates on Docker named volumes via a ``docker.DockerClient``.

    Handles reading, writing, and validating Claude Code OAuth credentials
    stored as ``.credentials.json`` inside the ``claude-auth`` volume, plus
    a few low-level volume ownership utilities that the deploy path also
    needs.
    """

    def __init__(self, client: Any) -> None:
        """*client* is a ``docker.DockerClient`` (or ``docker.APIClient`` for
        the low-level API used by ``_ensure_volume_ownership``).
        """
        self._client = client
    ...
    # -- credential check (used by deploy/rollback) -------------------------
