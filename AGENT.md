...
> **Config ownership:** runtime component config (`config/config.json` per
> component) is fully **component-owned** and edited through each component's
> own Settings API. All former `/services/{name}/config` endpoints
> (`GET`/`PUT`, `/export`, `/import`, `/refresh-schema`, `/assist`) have been
> **removed**. The deploy plane manages Docker-boundary settings only (image,
> ports, mounts, env/secrets).
