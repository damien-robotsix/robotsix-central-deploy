...
- `auto_update_enabled` — if true, the caretaker may automatically update this component; defaults to true
...
11. **Modifying `LifecycleConfig` requires syncing schema + example in the same commit** — whenever you add/remove a setting in `src/robotsix_central_deploy/lifecycle/config.py`, you MUST in the same commit: (1) re-generate `config/config.schema.json` via the documented generator (`uv run python -c "from robotsix_central_deploy.lifecycle.config import LifecycleConfig; import robotsix_config; print(robotsix_config.config_schema_json(LifecycleConfig))" > config/config.schema.json`), (2) add/remove the matching top-level key in `config/config.example.json`, and (3) ensure `config/config.schema.json` ends with exactly one trailing newline (no blank line) to satisfy the Grep-Lint `end-of-file-fixer`. CI's `config-schema-drift` gate will otherwise fail as a separate Red run. (Mirrors the openapi.json-in-same-commit discipline in the Documentation section.)
