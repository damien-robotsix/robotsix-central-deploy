...
- Fix startup ordering so ``system_settings.json`` overlay is applied *before* the backend is constructed.  Previously the backend was built from raw config, so restart-gated settings (``claude_host_mount_path``, ``docker_socket_url``, ``docker_sdk_timeout``) never reflected the persisted overlay even after a full service restart.  The PUT ``/settings`` docstring now accurately reflects the corrected behavior.
