...
- Add per-component deploy history: every successful deploy records a timestamped `DeployHistoryEntry` (digest, image ref, source, previous digest) in a new JSON store, capping at 20 entries per component. Exposed via `GET /services/{name}/history`. Rollback extended to accept an optional `{"digest": "..."}` body targeting any recorded digest.
